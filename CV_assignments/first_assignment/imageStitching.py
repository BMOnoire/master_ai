...
def get_matches(desc_list_1, desc_list_2, threshold):
    ...
    for idx_1, desc_1 in enumerate(desc_list_1):
        for idx_2, desc_2 in enumerate(desc_list_2):
            ...
            norm_desc_1 = (desc_1 - np.mean(desc_1)) / (np.std(desc_1))
            norm_desc_2 = (desc_2 - np.mean(desc_2)) / (np.std(desc_2))

            normal_cross_correlation = np.correlate(norm_desc_1, norm_desc_2) / (len(desc_1) - 1)
            if normal_cross_correlation > 0.5:

                euclidean_distance = np.linalg.norm(norm_desc_1 - norm_desc_2)

                if euclidean_distance < MATCH_THRESHOLD:
...
def ransac(keypoints_1, keypoints_2, matches):
    ...
    # homography between two sets of points
...
def image_stitcher(img_path_1, img_path_2):
    ...
    print("image 1 keypoints: ", len(harris_keypoints_1))
    print("image 2 keypoints: ", len(harris_keypoints_2))

    cv2.imshow("IMAGE 1", harris_img_1)
    cv2.imwrite('harris_img_1.jpeg', harris_img_1)
    cv2.imshow("IMAGE 2", harris_img_2)
    cv2.imwrite('harris_img_2.jpeg', harris_img_2)
    cv2.waitKey(0)
    ...
    if len(matches) < 4:
        print("Too few matches: ", len(matches))
        return 1
    ...
    if SHOW_CORNERS:
        ...
        cv2.imwrite('sift_img_1.jpeg', sift_img_1)
        cv2.imshow("IMAGE 2", sift_img_2)
        cv2.imwrite('sift_img_2.jpeg', sift_img_2)
        cv2.waitKey(0)
        cv2.destroyAllWindows()

    if SHOW_ALL:
        cv2.imshow("IMAGE 1", img_1)
        cv2.imwrite('img_1.jpeg', img_1)
        cv2.imshow("IMAGE 2", img_2)
        cv2.imwrite('img_2.jpeg', img_2)
        cv2.imshow("SHOW MATCHES", img_matching)
        cv2.imwrite('img_matching.jpeg', img_matching)
        cv2.imshow("TRANSFORMATION", img_transformed)
        cv2.imwrite('img_transformed.jpeg', img_transformed)
